...
def append_to_log(text):
    ...
    log_file = open("%s/%s" % (destination, log_file_name), 'a')
    log_file.write('\n'+text)
    log_file.close()
...
def present_warnings():
    ...
    if len(errors):
        ...
        log_file = open("%s/%s" % (destination, log_file_name), 'w')
        log_file.write('\n'.join(info) + '\n' + '\n'.join(errors))
        log_file.close()
...
def main():
    ...
    # 1 = input (source) root folder, 2=output (destination) root folder
    try:
        source = sys.argv[1]
    except:
        source = ''
    ...
    source = source or "/media/bruce/mega/x-solo-on-moto/media/footage"
    destination = destination or "/media/bruce/silver/solo-on-moto/media/proxies"

    # load config file (/dst/.config)
    ...
    source = raw_input("Original folder(%s): " % (source)) or source
    ...
    info = [
        ...
        tsv("INFO", "input", source),
        tsv("INFO", "output", destination),
    ...
    ignore_folders = get_ignored_folders(destination)
